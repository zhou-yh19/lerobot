...
def ensure_dir_available(src: Path, dst: Path) -> bool:
    """Expose a source directory under dst via symlink (preferred) or copytree (fallback)."""
    if not src.exists():
        logging.warning(f"Skip exposing missing directory: {src}")
        return False

    if dst.is_symlink() and not dst.exists():
        # Dangling symlink (e.g. the repo checkout moved since the last run).
        dst.unlink()
    if dst.exists():
        return True
    ...
    try:
        dst.symlink_to(src.resolve().as_posix(), target_is_directory=True)
        logging.info(f"Symlink created: {dst} -> {src}")
    except OSError:
...
def run_server(
    ...
    def show_episode(
        ...
        urdf_relpath = "urdf/urdf/robot.urdf"
        urdf_viewer = {
            "enabled": (static_folder / urdf_relpath).exists(),
            "urdf_url": url_for("static", filename=urdf_relpath),
            "package_url": url_for("static", filename="urdf"),
            "up": "+Z",
        }
...
def visualize_dataset_html(
    ...
    ensure_dir_available(tele_urdf_src, static_dir / "urdf")
